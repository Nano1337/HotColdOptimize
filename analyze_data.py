...
def to_modtimes(np_diff, proj_name, facility_name):
    ...
    for i in range(1, len(project_info[3])):
        modtimes.append(math.floor(project_info[3][i] - initial))
...
def get_sd(times):
    ...
    if not times:
        return np.std([1, 0, 0, 0, 0, 0, 0])
    else:
        data = np.zeros((times[len(times)-1]+1,), dtype=int)
        data[0] = 1
        for time in times:
            data[time] += 1
        return np.std(data)
...
if __name__ == "__main__":
    ...
    for i in range(len(np_diff)):
        np_diff[i, 3] = to_modtimes(np_diff, np_diff[i, 1], np_diff[i, 0])

    # Get standard deviations of each project
    sd = []
    for i in range(len(np_diff)):
        sd.append(get_sd(np_diff[i, 3]))

    # use optimal histogram method to display sd distribution
    optimize_bin(sd)
